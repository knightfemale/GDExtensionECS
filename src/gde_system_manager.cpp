...
void GdeSystemManager::_process(double delta) {
    for (int i = 0; i < systems.size(); ++i) {
        ...
        if (system->has_method("_system_process")) {
            // 获取组件映射并转换为Dictionary
            auto components_map = get_components_for_system(system);
            Dictionary components;

            for (const auto& pair : components_map) {
                Array arr;
                for (GdeComponent* comp : pair.second) {
                    arr.append(comp);
                }
                components[String(pair.first.c_str())] = arr;
            }
...
std::unordered_map<std::string, std::vector<GdeComponent*>> GdeSystemManager::get_components_for_system(GdeSystem* system) {
    std::unordered_map<std::string, std::vector<GdeComponent*>> components_map;
    ...
    for (size_t entity_id : active_entities) {
        ...
        for (const std::string& name : required) {
            ...
            if (comp) components_map[name].push_back(comp);
        }
    }
